...
class GenrePagingDataSourceTest {
    @get:Rule
    val rule = InstantTaskExecutorRule()

    private lateinit var sut: GenrePagingDataSource
    ...
    fun setUp() {
        ...
        sut = GenrePagingDataSource(apiService, baseModelMapper, genreId)
    }
    ...
    fun `load returns Page on successful load`() = runTest {
        testScope.launch {
            ...
            coEvery { apiService.moviesByGenre(1, genreId) } returns baseModelEntity
            coEvery {
                safeApiCall {
                    apiService.moviesByGenre(
                        1,
                        genreId
                    )
                }
    ...
    fun `load returns Error on IOException`() = runTest {
        ...
        coEvery { apiService.moviesByGenre(1, genreId) } throws mockException
        coEvery { safeApiCall { apiService.moviesByGenre(1, genreId) } } returns mockDataStateError
