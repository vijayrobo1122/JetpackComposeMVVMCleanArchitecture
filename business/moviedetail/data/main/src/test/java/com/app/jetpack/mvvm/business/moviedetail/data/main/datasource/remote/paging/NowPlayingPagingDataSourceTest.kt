...
class NowPlayingPagingDataSourceTest {
    @get:Rule
    val rule = InstantTaskExecutorRule()

    private lateinit var sut: NowPlayingPagingDataSource
    ...
    fun setUp() {
        ...
        sut = NowPlayingPagingDataSource(apiService, baseModelMapper, genreId)
    }
    ...
    fun `load returns Page on successful load`() = runTest {
        testScope.launch {
            ...
            coEvery { apiService.nowPlayingMovieList(1, genreId) } returns baseModelEntity
            coEvery {
                safeApiCall {
                    apiService.nowPlayingMovieList(
                        1,
                        genreId
                    )
                }
    ...
    fun `load returns Error on IOException`() = runTest {
        ...
        coEvery { apiService.nowPlayingMovieList(1, genreId) } throws mockException
        coEvery {
            safeApiCall {
                apiService.nowPlayingMovieList(
                    1,
