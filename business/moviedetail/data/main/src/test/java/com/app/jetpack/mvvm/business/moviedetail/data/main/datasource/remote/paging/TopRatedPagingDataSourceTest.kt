...
class TopRatedPagingDataSourceTest {
    @get:Rule
    val rule = InstantTaskExecutorRule()

    private lateinit var sut: TopRatedPagingDataSource
    ...
    fun setUp() {
        ...
        sut = TopRatedPagingDataSource(apiService, baseModelMapper, genreId)
    }
    ...
    fun `load returns Page on successful load`() = runTest {
        testScope.launch {
            ...
            coEvery { apiService.topRatedMovieList(1, genreId) } returns baseModelEntity
            coEvery {
                safeApiCall {
    ...
    fun `load returns Error on IOException`() = runTest {
        ...
        coEvery { apiService.topRatedMovieList(1, genreId) } throws mockException
        coEvery {
            safeApiCall {
                apiService.topRatedMovieList(
                    1,
