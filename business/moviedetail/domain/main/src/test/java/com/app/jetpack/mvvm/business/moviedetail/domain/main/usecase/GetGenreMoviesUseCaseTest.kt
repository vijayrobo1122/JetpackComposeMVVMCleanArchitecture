...
class GetGenreMoviesUseCaseTest {

    @get:Rule
    val rule = InstantTaskExecutorRule()

    private lateinit var sut: GetGenreMoviesUseCase
    ...
    fun setUp() {
        ...
        sut = GetGenreMoviesUseCase(repository)
    }
    ...
    fun `given genreId when usecase invoke should return paging data`() {
        testScope.launch {
            ...
            every { repository.genrePagingDataSource(genreId) } returns successFlow
            ...
            // Then
            result.test {
                verify { repository.genrePagingDataSource(genreId) }
    ...
    fun `given genreId when usecase invoke emits empty PagingData when repository returns empty data`() {
        testScope.launch {
            ...
            every { repository.genrePagingDataSource(genreId) } returns emptyFlow
    ...
    fun `given genreId when usecase invoke throws exception when repository throws exception`() {
        testScope.launch {
            ...
            every { repository.genrePagingDataSource(genreId) } returns flow { throw error }
