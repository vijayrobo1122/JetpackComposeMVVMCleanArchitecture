...
class GetPopularMoviesUseCaseTest {
    ...
    fun `given genreId when usecase invoke should return paging data`() {
        testScope.launch {
            ...
            every { repository.popularPagingDataSource(genreId) } returns successFlow
            ...
            // Then
            result.test {
                verify { repository.popularPagingDataSource(genreId) }
    ...
    fun `given genreId when usecase invoke emits empty PagingData when repository returns empty data`() {
        testScope.launch {
            ...
            every { repository.popularPagingDataSource(genreId) } returns emptyFlow
    ...
    fun `given genreId when usecase invoke throws exception when repository throws exception`() {
        testScope.launch {
            ...
            every { repository.popularPagingDataSource(genreId) } returns flow { throw error }
