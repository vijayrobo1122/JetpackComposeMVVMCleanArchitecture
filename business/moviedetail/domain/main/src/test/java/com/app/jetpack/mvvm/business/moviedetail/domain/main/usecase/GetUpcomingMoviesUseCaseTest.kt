...
class GetUpcomingMoviesUseCaseTest {
    ...
    fun setUp() {
        ...
        sut = GetUpcomingMoviesUseCase(repository)
    }
    ...
    fun `given genreId when usecase invoke should return paging data`() {
        testScope.launch {
            ...
            every { repository.upcomingPagingDataSource(genreId) } returns successFlow
            ...
            // Then
            result.test {
                verify { repository.upcomingPagingDataSource(genreId) }
    ...
    fun `given genreId when usecase invoke emits empty PagingData when repository returns empty data`() {
        testScope.launch {
            ...
            every { repository.upcomingPagingDataSource(genreId) } returns emptyFlow
    ...
    fun `given genreId when usecase invoke throws exception when repository throws exception`() {
        testScope.launch {
            ...
            every { repository.upcomingPagingDataSource(genreId) } returns flow { throw error }
